use super::completed_item_crosses_answer_boundary;
...
#[test]
fn completed_item_crosses_answer_boundary_for_unknown_phase_messages() {
    let item = assistant_output_text("final answer");

    assert!(completed_item_crosses_answer_boundary(
        &item, /*plan_mode*/ false
    ));
}

#[test]
fn completed_item_does_not_cross_answer_boundary_for_commentary_messages() {
    let item = assistant_output_text_with_phase("working on it", Some(MessagePhase::Commentary));
