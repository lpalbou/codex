...
const TURN_2_PROMPT: &str = "follow up without wait";
...
async fn setup_turn_one_with_spawned_child(server: &MockServer) -> Result<TestCodex> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            anyhow::bail!("timed out waiting for parent rollout to include subagent notification");
        }
        sleep(Duration::from_millis(10)).await;
    }
...
async fn subagent_notification_is_included_without_wait() -> Result<()> {
    ...
    let test = setup_turn_one_with_spawned_child(&server).await?;

    let turn2 = mount_sse_once_match(
        &server,
        |req: &wiremock::Request| body_contains(req, TURN_2_PROMPT),
        sse(vec![
    ...
    .await;
    test.submit_turn(TURN_2_PROMPT).await?;
